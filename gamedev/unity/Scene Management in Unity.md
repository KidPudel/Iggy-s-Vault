...
## Accessing Persistent Scene Managers

Managers in the Persistent scene are **not singletons** — they're regular MonoBehaviours. They survive because their scene isn't unloaded, not because of static references.

**How other scripts access them:**

|Approach|How|Tradeoff|
|---|---|---|
|Service Locator|Manager registers itself in `Awake()`; others call `Services.Get<T>()`|Manual, pull-based|
|Dependency Injection|DI container finds manager, injects into classes that need it|Automatic, push-based|

```csharp
// Service Locator approach — manager registers itself
public class AudioManager : MonoBehaviour, IAudioService
{
    private void Awake() => Services.Register<IAudioService>(this);
    private void OnDestroy() => Services.Unregister<IAudioService>();
}

// DI approach — container finds and wires it
// In LifetimeScope:
builder.RegisterComponentInHierarchy<AudioManager>().As<IAudioService>();
```

See [[Communication Patterns in Unity]] for Service Locator vs DI details and problem of Singletons.
